...
**Status.** Phases A, B and C shipped. Phase A: `packages/design` owns the tokens and the six primitives every component is built from, and no component hard-codes a color. Phase B shipped: the floating transport, the library drawer, and dim-on-play, all inherited automatically by share-link recipients. Phase C's keystone shipped: the Cmd+K palette assembles its commands from live context, which is what lets visible chrome shrink. Reduced motion and a visible focus ring are in.

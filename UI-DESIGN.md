...
Phase E is under way: two of the three signature moments are in. **Play**: the transport's play control is a progress ring — the only round thing on the surface, so it needs no caption — and the position bar beside it is scrubbable, which it never was despite looking exactly like it should be. **Share**: the card springs in from the press, copies the link without being asked, and carries a live miniature of the score's first system, so what you are about to send looks like music rather than a random string. Joining a session, the third, is not done.

...
CubScore is a standalone platform for making music with notation and tablature: write it, play it, practice against it, perform from it, and work on it live with other people, all from a browser. Web-first and AI-native, built and hosted on CubCloud sovereign infrastructure in Missoula, Montana.

The platform stands on its own model and its own tools. Every document lives in CubScore's semantic score model with a full operation log, which is what makes realtime collaboration, inverse-operation undo, and the practice and transcription engines possible. Other software's formats are supported as interchange, because your music should never be trapped anywhere, including here: CubScore reads and writes MusicXML, MIDI, Guitar Pro, alphaTex, and ASCII tab, and grades its own fidelity on every one of them.
...
- **Editor** — fret entry on a semantic score model with a full operation log. Type `0`-`9` for frets (two quick digits combine into frets 10-24) — or, on a drum staff, for kit voices, with a legend strip because a percussion staff has no frets to make the digits self-evident and the same key again takes the hit back off. Arrows move the caret, `+`/`-` for beats, `Enter` for a bar, `Ctrl+Z`/`Ctrl+Shift+Z` for undo and redo, plus durations, dots, and articulations. Tracks are an instrument rail beside the score; tempo and meter are chips on the caret's bar, which is framed in the score so you can see where you are. Work autosaves to the library and reopens in the editor.
...
- **Import** — MusicXML, MIDI (`.mid`, `.midi`), Guitar Pro (`.gp`, `.gp3`, `.gp4`, `.gp5`, `.gpx`), CapXML, alphaTex, and ASCII tablature (`.txt`, `.tab`), by file picker or drag-and-drop. ASCII tab is read by CubScore itself: it recovers strings, frets and bars from a file somebody typed by hand, ignores the lyrics and chord names around the staff, reads the tuning off the string labels or out of the prose, and reports that rhythm was not carried — because the format records none, and a plausible wrong rhythm is worse than an honest even one.
- **Export** — MusicXML, MIDI, Guitar Pro `.gp`, alphaTex, ASCII tablature, and print/PDF. ASCII tablature goes to the clipboard as well as a file, because pasting is the whole point of that format — with spacing proportional to duration, since spacing is the only rhythm information ASCII tab can carry. MIDI is written by CubScore itself (`packages/formats/src/to-midi.ts`) from the semantic model, so the file carries the document you edited — repeats expanded, tempo and meter mapped, ties held rather than re-articulated. `pnpm midi` grades it against alphaTab's own MIDI on every score in the corpus.
...
Not built yet: offline collaboration merge (CRDT) and all AI features. (Password reset exists, by recovery code. Email confirmation exists; it proves an address rather than restricting an account, and it cannot displace a squatter who registered one first — see DEPLOY.md.)
...
Imported files are editable, and editing one never costs you the original. The library row keeps the source bytes, filename, and import report untouched — autosave only ever writes the fields the editor owns — so an import carries both the original file and your working edit. "Show imported original" in the palette switches which one opening shows; EDIT resumes the edit.

An import is converted to the semantic model, and pressing EDIT opens it with a report of anything the model could not carry. Multiple voices per bar and ties survive the conversion and round-trip exactly. Percussion is carried whole: drum tracks come into the semantic model as General MIDI drum voices, are written to MIDI on channel 10, and are engraved as notation like any other part, so a drum-only file is editable rather than play-only. Getting the notation right meant establishing what alphaTex actually wants, which is not what an earlier attempt concluded: a number on a percussion staff is an index into a list the renderer builds from the *names* a file used, assigned in order of first appearance, which is why writing drum numbers produced a file that rendered a full kit and played five sounds. `packages/core/src/percussion.ts` is the bridge, and every one of its 51 rows was measured by writing the name into a one-note staff and reading back the drum it sounded. Percussion is now inside the round-trip comparison `pnpm corpus` makes, so a name that sounds the wrong drum is pitch drift on every corpus score with a kit in it. Alternate endings, chord diagrams, section markers, and detailed bend curves are reported the same way.

A file with nothing CubScore can edit stays play-only and says so in the player rather than offering EDIT and handing you a blank guitar staff where your music had been. A drum-only transcription used to be that case and no longer is, and neither is an imported piano or vocal part: a staff with no strings takes scale degrees of the bar's key on the number row, so 1 is the tonic and 5 the dominant in whatever key the music is actually in, with a strip naming the pitch each degree lands on in the caret's octave.
...
**Transpose is one command and one undo.** The palette moves the whole document by a semitone or a tone, either way, and all three statements of the music move together: pitches shift, every fretted note is refingered from scratch by the solver (a shape slid two frets is wrong the moment it used an open string), and the chord chart is respelled for the destination key, so a chart moved into F reads Bb rather than A#. An interval that would push a part off its instrument's neck is refused with the reason and the direction that fits, because a score whose guitar moved and whose bass did not is two keys at once. One Ctrl+Z takes the whole key change back.

**Chord diagrams are computed, not looked up.** While editing, the chart's distinct chords appear as fingering grids drawn by CubScore from the same voicing engine that powers chord entry, so the diagrams are correct in DADGAD, drop D, or under a capo, where a dictionary of standard-tuning shapes goes quietly wrong. A symbol the grammar cannot read is listed by name with no picture, because an invented shape is worse than none.

**The practice plan is a button, not a diagnosis.** The strip already names the bars to work on and the tempo the record says you can actually play; clicking a named bar now sets the loop region to that bar and the playback speed to that tempo. Measurement becomes action in one click, and the speed only ever recommends down: this is practice, not a victory lap.

**Version history, on by default.** While you edit, the library keeps a snapshot of the document about once a minute, twenty per score. RESTORE makes any of them current again through the ordinary save path, so it bumps the revision and survives reload; later versions are kept, so a restore you regret is itself recoverable. Deleting a score deletes its history.

**Accounts recover without an email service.** Signup shows a recovery code once (grouped, unambiguous alphabet, stored only as a hash) — the email-less password reset, which is not a shortcut but the point: an account system on sovereign infrastructure should not depend on somebody else's SMTP to give a user their account back. Using the code sets a new password, issues a fresh code, and ends every other session the account has, because the code gets used exactly when the password may be in someone else's hands.

**And it sends mail without a mail provider.** Confirming an address does need a message to reach it, so that one promise bends — but only that far: nothing in `services/api/src/mail.ts` knows what a mail provider is. It composes an RFC 5322 message and hands it to one of two transports. `file` writes it under the data directory, mode 0600, which is the honest behaviour for a single machine with no mail configured and is the same path the e2e suite drives. `command` pipes it to `sendmail -t`, `msmtp`, or a provider's own CLI. No SMTP client, no SDK, no API key in the deployment, no outbound connection to a third party on every signup. Confirmation is off entirely unless `PUBLIC_URL` is set, because the alternative source for a link's origin is the request's `Host` header, and that would let anyone reachable by the API mail a victim a link to a host of their choosing carrying a live token for the victim's account.
...
pnpm e2e        # drive the built app in a browser across twenty journeys

...
- **Editor** — fret entry on a semantic score model with a full operation log. Type `0`-`9` for frets (two quick digits combine into frets 10-24), arrows to move the caret, `+`/`-` for beats, `Enter` for a bar, `Ctrl+Z`/`Ctrl+Shift+Z` for undo and redo, plus durations, dots, and articulations. Tracks are an instrument rail beside the score; tempo and meter are chips on the caret's bar, which is framed in the score so you can see where you are. Work autosaves to the library and reopens in the editor.
